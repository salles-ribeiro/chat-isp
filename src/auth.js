...
// ── Criar usuário ──────────────────────────────────────────────────────────────
// DEPOIS — envolva apenas o bloco de DB em try/catch (mantenha validações fora):
async function criarUsuario({
  ...
  if (!nomeCompleto || nomeCompleto.trim().length < 3)
    return { erro: "Nome completo inválido." };
  if (!cpf || !validarCPF(cpf)) return { erro: "CPF inválido." };
  if (!email || !email.includes("@")) return { erro: "E-mail inválido." };
  if (!nomeExibicao || nomeExibicao.trim().length < 2)
    return { erro: "Nome de exibição inválido." };
  ...
  try {
    ...
    const existeNome = await prisma.usuario.findUnique({
      where: { nomeExibicao: nomeLimpo },
    });
    ...
    await prisma.usuario.create({
      data: {
        ...
        equipe: (equipe || "").trim().substring(0, 30),
    ...
    console.error("[criarUsuario]", e.message);
    return { erro: "Erro ao salvar usuário. Tente novamente." };
  }
}
// ── Editar usuário ─────────────────────────────────────────────────────────────
  ...
  if (quemEdita.admin) {
    ...
    if (dados.email !== undefined) {
      ...
      const dup = await prisma.usuario.findFirst({
        where: { email: emailLimpo, id: { not: cpfAlvo } },
      });
    ...
    if (dados.nomeExibicao !== undefined) {
      ...
      const dup = await prisma.usuario.findFirst({
        where: { nomeExibicao: nomeLimpo, id: { not: cpfAlvo } },
      });
      if (dup) return { erro: "Nome de exibição já em uso." };
      update.nomeExibicao = nomeLimpo;
    }
    if (dados.admin !== undefined) {
      if (alvo.id === quemEdita.cpf && !dados.admin)
        return { erro: "Não pode remover seu próprio acesso de admin." };
